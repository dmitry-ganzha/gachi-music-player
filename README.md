   ...
   [
      {
         ...
         //Сам аргумент, если указывать args то необходимо чтоб в конце аргумента было =
         //Пример atempo=
         ...
         //Ускоряется ли музыка если да то как (arg - ускоряется аргументом, 1.25 - ускоряется в 1.25)
         "speed": "arg"
      }  
   ]
     ```