...
export class Play extends Command {
    ...
    public readonly run = (message: ClientMessage, args: string[]): ResolveData => {
        ...
        if (!memberVoice?.channel || !memberVoice) return { text: `${author}, Подключись к голосовому каналу!`, color: "DarkRed" };

        //Если есть очередь и пользователь не подключен к тому же голосовому каналу
        if (queue && queue.voice && memberVoice?.channel?.id !== queue.voice.id) return {
