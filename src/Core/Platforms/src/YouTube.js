...
const Utils_1 = require("./youtube/Utils");
const decipher_1 = require("./youtube/decipher");
const VerAuthor = new Set(['Verified', 'Official Artist Channel']);
const DefaultLinkYouTube = 'https://www.youtube.com';
exports.YouTube = { getVideo, getPlaylist, SearchVideos };
function getChannel({ id, name }) {
    return new Promise(async (resolve) => {
        const channel = (await Promise.all([httpsClient_1.httpsClient.parseJson(`${DefaultLinkYouTube}/channel/${id}/channels?flow=grid&view=0&pbj=1`, {
        ...
        return resolve({
            ...
            isVerified: !!badges?.find((badge) => VerAuthor.has(badge?.metadataBadgeRenderer?.tooltip))
        });
    });
}
function getVideo(url, options = { onlyFormats: false }) {
    return new Promise(async (resolve, reject) => {
        const VideoID = new Utils_1.Utils().getID(url);
        const body = (await Promise.all([httpsClient_1.httpsClient.parseBody(`${DefaultLinkYouTube}/watch?v=${VideoID}&has_verified=1`, {
                options: { userAgent: true, zLibEncode: true, english: true, cookie: true }
            })]))[0];
        if (body.includes('Our systems have detected unusual traffic from your computer network.'))
            throw reject(new Error('Google понял что я бот! Это может занять много времени!'));
        let Token = body.match(/(["'])ID_TOKEN\1[:,]\s?"([^"]+)"/);
        const VideoRes = await httpsClient_1.httpsClient.parseJson(`${DefaultLinkYouTube}/watch?v=${VideoID}?flow=grid&view=0&pbj=1`, {
        ...
        if (!VideoFinalData)
            throw reject(new Error('Данные на странице не были найдены'));
        if (VideoFinalData.playabilityStatus?.status !== 'OK')
            throw reject(new Error(`Не удалось получить данные из-за: ${VideoFinalData.playabilityStatus.status}`));
        const html5player = `https://www.youtube.com${body.split('"jsUrl":"')[1].split('"')[0]}`;
        const videoDetails = VideoFinalData.videoDetails;
        let format;
        const LiveData = {
            isLive: videoDetails.isLiveContent,
            url: VideoFinalData.streamingData?.hlsManifestUrl ?? null
        };
        const VideoFormats = [...VideoFinalData.streamingData.formats ?? [], ...VideoFinalData.streamingData.adaptiveFormats ?? []].filter((d) => d?.mimeType?.match(/opus/) || d?.mimeType?.match(/audio/)) ?? [];
        if (!LiveData.isLive) {
            if (VideoFormats[0].signatureCipher || VideoFormats[0].cipher)
                format = (await Promise.all([(0, decipher_1.Decipher)(VideoFormats, html5player)]))[0];
            else
                format = [...VideoFormats];
        }
        else
            format = [{ url: LiveData.url, work: true }];
        if (options?.onlyFormats)
            return resolve(format.pop());
...
function SearchVideos(search, options = { limit: 15 }) {
    return new Promise(async (resolve, reject) => {
        const SearchRes = await httpsClient_1.httpsClient.parseJson(`${DefaultLinkYouTube}/results?search_query=${search.replaceAll(' ', '+')}?flow=grid&view=0&pbj=1`, {
            options: { userAgent: true, YouTubeClient: true, zLibEncode: true, english: true }
        });
        const SearchFinal = SearchRes.find((res) => res.response !== undefined).response;
...
function parsingVideos(details, { limit }, FakeBase = []) {
    let num = 0;
...
async function getPlaylist(url) {
    return new Promise(async (resolve, reject) => {
        const playlistID = new Utils_1.Utils().getID(url, true);
        const body = (await Promise.all([httpsClient_1.httpsClient.parseBody(`${DefaultLinkYouTube}/playlist?list=${playlistID}`, {
                options: { userAgent: true, zLibEncode: true, english: true, cookie: true }
            })]))[0];
        if (body.includes('Our systems have detected unusual traffic from your computer network.'))
            throw reject(new Error('Google понял что я бот! Это может занять много времени!'));
...
function _parsingVideos(parsed, finder = []) {
    let num = 0;
    for (let i in parsed) {
