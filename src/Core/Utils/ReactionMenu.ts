...
export class CollectorSortReaction {
     ...
     */
    public _run = async (embed: EmbedConstructor | string, pages: any[], page: number, message: ClientMessage, EnableQueue: boolean): Promise<ReactionCollector> => message.channel.send(typeof embed === "string" ? embed : {embeds: [embed]}).then(async (msg: ClientMessage) => {
    ...
    protected _callback_string = async (page: number, pages: any[], queue: Queue): Promise<CollectorSortReactionFunction> => {
        return {
            ...
            cancel: async (reaction: MessageReaction, user: User, message: ClientMessage, msg: ClientMessage): Promise<NodeJS.Timeout> => setTimeout(async () => (await this.DeleteMessage(msg), await this.DeleteMessage(message)), 50),
     ...
     */
    protected _reaction = async (user: User, message: ClientMessage, msg: ClientMessage, callback: Function, emoji: string): Promise<ReactionCollector> => msg.react(emoji).then(async () => msg.createReactionCollector({filter: async (reaction: MessageReaction, user: User) => this._filter(emoji, reaction, user, message), time: 1e4 * 25})
